...
<html>
<head>
<link href="http://npmcdn.alibaba-inc.com/normalize.css@4.1.1" media="all" rel="stylesheet" />
<script src="http://npmcdn.alibaba-inc.com/currentscript"></script>
<script src="http://npmcdn.alibaba-inc.com/babel-polyfill@6.9.1/dist/polyfill.min.js"></script>
<script src="http://npmcdn.alibaba-inc.com/jquery@3.1.1"></script>
</head>
<body>
  <style>
    body { margin: 0; padding: 30px 40px; font-size: 12px; line-height: 1.5; }
    #mountNode:empty:after {
      ...
      bottom: 0;
      content: '';
      background: url(https://os.alipayobjects.com/rmsportal/RinPJBDfFDAEeGh.png) center 40% no-repeat;
...
class Sandbox {
  ...
  renderTo(container, globals, options) {
    container.appendChild(this.node)
    this.node.contentWindow.document.write(initialContent)
  ...
  loadExtenal(moduleName, callback) {
    ...
    if (moduleName.indexOf('.css') > 0) {
      loadCss({
        href: `http://unpkg.com/${moduleName}`,
        class: 'npmcdn-links',
        id: `npmcdn-links-${moduleName}`,
      })
      return
    }
    loadScript(`http://unpkg.com/${this.transforModuleMain(moduleName)}`, {
      attrs: {
        class: 'npmcdn-links',
        id: `npmcdn-links-${moduleName}`,
      },
